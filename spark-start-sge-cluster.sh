...
while getopts "d:e:c:m:p:l:" opt; do
  case ${opt} in
    ...
    l ) MEMORY_RESOURCE=$OPTARG
      ;;
    \? )
      echo "Usage: $0 [-d] output_dir [-e] executors [-c] cpu_per_executor [-m] gb_mem_per_executor [-p] Parallel_env [-l] memory_resource"
      exit 1
      ;;
  esac
done
...
spark-class org.apache.spark.deploy.master.Master 2>> $LOG_DIR/master.log >> $LOG_DIR/master.log &
...
-d $PWD 2>> $LOG_DIR/executor-${i}.log >> $LOG_DIR/executor-${i}.log &
